...
export default function BarcodeGenerator() {
  ...
  const generateBarcode = () => {
    ...
    if (!txt) { setMsg("Ingresá un texto primero.", "error"); return; }
    setBatchTexts([]);
    setCurrentText(txt);
    setAnimate(false);
    setTimeout(() => { setGenerated(true); setAnimate(true); }, 10);
    setMsg("Código generado correctamente.", "success");
  };

  const generateBatch = () => {
    const lines = batchText.split("\n").map(l => l.trim()).filter(l => l);
    if (!lines.length) { setMsg("Ingresá al menos un texto.", "error"); return; }
    setBatchTexts(lines);
    setCurrentText(lines[0]);
    setAnimate(false);
    setTimeout(() => { setGenerated(true); setAnimate(true); }, 10);
    setMsg(`${lines.length} código${lines.length > 1 ? "s" : ""} listos para exportar.`, "success");
  };
    ...
    new Promise((resolve) => {
      ...
      try {
        window.JsBarcode(el, text, {
          format: "CODE128", lineColor: "#000000", width: 2.2, height: 90,
          displayValue: true, fontSize: 13, margin: 12, background: "#ffffff",
        });
      } catch { document.body.removeChild(el); resolve(null); return; }
      ...
      img.onload = () => {
        ...
        canvas.width = img.width || 420; canvas.height = img.height || 130;
  ...
  const downloadWord = async () => {
    if (!docxReady || !jsBarcodeReady) { setMsg("Cargando librerías...", ""); return; }
    setLoading(true);
    ...
    for (let i = 0; i < texts.length; i++) {
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: txt, bold: true, size: 28, font: "Courier New" })],
        spacing: { after: 160 },
      }));
      ...
      if (dataUrl) {
        ...
        children.push(new Paragraph({
          children: [new ImageRun({ data: bytes, transformation: { width: 400, height: 120 }, type: "png" })],
  ...
  const printBarcode = () => {
    ...
    const entries = texts.map((t, i) => `<div class="entry" id="e${i}"><div class="label">${t.replace(/</g,"&lt;")}</div><svg id="bc${i}"></svg></div>`).join("");
    win.document.write(`<!DOCTYPE html><html><head><title>Códigos</title>
<style>body{font-family:monospace;padding:24px;background:#fff}
.entry{margin-bottom:40px;page-break-inside:avoid}
.label{font-size:18px;font-weight:bold;margin-bottom:10px;color:#0f172a}
.btn{padding:10px 20px;background:#0f172a;color:#fff;border:none;cursor:pointer;border-radius:6px;margin-bottom:20px;font-size:14px}
@media print{.btn{display:none}}</style></head><body>
<button class="btn" onclick="window.print()">Imprimir</button>
${entries}
<script src="${JSBARCODE_URL}"><\/script>
<script>window.onload=function(){${texts.map((t, i) => `try{JsBarcode(document.getElementById('bc${i}'),'${t.replace(/'/g,"\\'")}',{format:'CODE128',lineColor:'#000',width:2.2,height:90,displayValue:true,fontSize:13,margin:12,background:'#fff'})}catch(e){}`).join(";")}}<\/script>
</body></html>`);
    win.document.close();
  };

  const displayTexts = batchTexts.length ? batchTexts : currentText ? [currentText] : [];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-mono">
      ...
      <div className="border-b border-slate-800 px-6 py-5 flex items-center gap-4">
        <div className="flex gap-1">
          {[..."▮▯▮▮▯▮▯▯▮▮"].map((c, i) => (
            <span key={i} className={`text-lg ${c === "▮" ? "text-cyan-400" : "text-slate-700"}`}>{c}</span>
          ))}
        </div>
        <div>
          <h1 className="text-lg font-bold tracking-widest text-white uppercase">Code-128 Generator</h1>
          <p className="text-xs text-slate-500 tracking-wider">Impresión directa · Exportación Word</p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* Status */}
        {status.msg && (
          <p className={`text-xs tracking-wider ${status.type === "error" ? "text-red-400" : status.type === "success" ? "text-emerald-400" : "text-slate-400"}`}>
            {status.type === "error" ? "✗ " : status.type === "success" ? "✓ " : "⋯ "}{status.msg}
          </p>
        )}
        ...
        {generated && displayTexts.length > 0 ? (
          <div className={`transition-all duration-500 ${animate ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            ...
            <div className="space-y-4">
              {displayTexts.map((txt, i) => (
                ...
                >
                  <p className="text-slate-950 font-bold text-base tracking-widest mb-3 font-mono">{txt}</p>
                  <div className="bg-white">
                    {jsBarcodeReady && <BarcodePreview text={txt} barcodeReady={jsBarcodeReady} />}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="border border-dashed border-slate-800 rounded-xl p-12 text-center">
            <div className="flex justify-center gap-1 mb-4">
              {[..."▮▯▮▮▯▮▯▯▮▮▯▮"].map((c, i) => (
                <span key={i} className={`text-2xl ${c === "▮" ? "text-slate-700" : "text-slate-800"}`}>{c}</span>
              ))}
            </div>
            <p className="text-slate-600 text-sm tracking-wider">Ingresá un texto para generar el código</p>
          </div>
        ) : null}
      </div>
    </div>
